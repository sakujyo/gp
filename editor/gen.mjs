...
function _gen(n, il = 0, decorator) {
	...
	switch (n.type){
		...
		case 'ExportNamedDeclaration':
			return `export ${n.attributes.map(x => gen(x).join(' '))}${n.specifiers.map(x => gen(x)).join('')}${n.declaration ? gen(n.declaration) : ''}${n.source ? ` from ${gen(n.source)}` : ''}`
		case 'ExportAllDeclaration':
			return `export * from ${gen(n.source)}`
